...
def part2(data):
    ...
    gears = []
    for line_number, line_data in enumerate(data):
        for match in re.finditer(r'\*', line_data):
            gears.extend([Gear(line_number, match.start(), data)])

    for gear in gears:
        # gear.show_output()
        output_list = gear.get_output_v2()
        if len(output_list) == 2 and output_list[0] != '' and output_list[1] != '':
            gear_ratio = int(output_list[0]) * int(output_list[1])
            print(output_list, gear_ratio)
